...
func CheckGPUSupport(gpuType deviceinfo.GPUType) error {
	...
	case deviceinfo.Others:
		return fmt.Errorf("failed to get the GPU type info: unrecognized GPU")
	default:
		return fmt.Errorf("unsupported GPU type %s for Confidential Computing (only supported for H100[a3-highgpu-1g], B200[a4-highgpu-8g], and RTX PRO 6000[g4-standard])", gpuType.String())
	}
	return nil
}
